...
export default function LoginPage() {
  ...
  const handleSuperAdminSignup = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Starting super admin signup fetch...');
      // Call the server-side API for super admin signup
      let response;
      try {
        response = await fetch('/api/auth/super-admin-signup', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      try {
        ...
        setError(`Server error: ${responseText || 'Empty response'}`);
        setLoading(false);
        return;
      }

      if (!response.ok) {
        const errorMsg = result.details || result.error || 'Super admin signup failed';
        const fullError = result.code ? `${errorMsg} (Code: ${result.code})` : errorMsg;
        setError(fullError);
        setLoading(false);
        return;
      }
    ...
    <ErrorBoundary>
      <div className="auth-page">
        ...
        <form onSubmit={isSuperAdminSignup ? handleSuperAdminSignup : handleLogin} className="auth-form">
          ...
          >
            {loading ? t('loading') : isSuperAdminSignup ? t('superAdminSignup') : 'Log In'}
          </button>
        </form>
