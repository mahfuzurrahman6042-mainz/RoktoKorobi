...
import { Building2, Edit, Trash2, Users, MapPin, Mail, Phone, Calendar } from 'lucide-react';
...
export default function OrganizationTable({ organizations, totalCount, currentPage, limit }: OrganizationTableProps) {
  ...
  const handleToggleStatus = async (organizationId: string, isActive: boolean) => {
    try {
      const response = await fetch('/api/admin/organizations/status', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ organizationId, active: !isActive })
      });
            ...
            <tr key={org.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
              ...
              <td className="px-6 py-4">
                <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <span className="truncate max-w-[150px]">{org.city || 'N/A'}</span>
                </div>
              </td>
              <td className="px-6 py-4">
                <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                  <Users className="w-4 h-4" />
                  <span>{org.hospital_count || 0}</span>
                </div>
              </td>
              <td className="px-6 py-4">
                {org.is_active ? (
              ...
              <td className="px-6 py-4">
                <div className="flex items-center gap-2">
                  ...
                  >
                    {org.is_active ? <Building2 className="w-4 h-4" /> : <Building2 className="w-4 h-4" />}
                  </button>
                  <button
